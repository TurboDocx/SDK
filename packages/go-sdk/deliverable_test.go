...
func TestDeliverableClient_ListDeliverableItems(t *testing.T) {
	t.Run("lists deliverable items", func(t *testing.T) {
		client := newDeliverableTestClient(t, func(w http.ResponseWriter, r *http.Request) {
			assert.Equal(t, "GET", r.Method)
			assert.Contains(t, r.URL.Path, "/v1/deliverable-item")

			w.Header().Set("Content-Type", "application/json")
			json.NewEncoder(w).Encode(DeliverableItemListResponse{
				Results:      []DeliverableItem{{ID: "item1", Name: "Item A", Type: "deliverable"}},
				TotalRecords: 1,
			})
		})

		result, err := client.ListDeliverableItems(context.Background(), &ListDeliverableItemsOptions{
			Limit:    10,
			ShowTags: true,
		})
		require.NoError(t, err)
		assert.Equal(t, 1, result.TotalRecords)
		assert.Equal(t, "item1", result.Results[0].ID)
	})
}

func TestDeliverableClient_GetDeliverableItem(t *testing.T) {
	t.Run("gets deliverable item", func(t *testing.T) {
		client := newDeliverableTestClient(t, func(w http.ResponseWriter, r *http.Request) {
			assert.Equal(t, "GET", r.Method)
			assert.Equal(t, "/v1/deliverable-item/item1", r.URL.Path)

			w.Header().Set("Content-Type", "application/json")
			json.NewEncoder(w).Encode(DeliverableItemResponse{
				Results: DeliverableItem{ID: "item1", Name: "Item A"},
				Type:    "deliverable",
			})
		})

		result, err := client.GetDeliverableItem(context.Background(), "item1", nil)
		require.NoError(t, err)
		assert.Equal(t, "item1", result.Results.ID)
		assert.Equal(t, "deliverable", result.Type)
	})
}
