package partner

import (
	"context"
	"fmt"

	"github.com/TurboDocx/SDK/packages/cli/cmd/cmdutil"
	"github.com/TurboDocx/SDK/packages/cli/internal/output"
	"github.com/spf13/cobra"
)

var orgUserRemoveCmd = &cobra.Command{
	Use:   "remove <orgId> <userId>",
	Short: "Remove user from organization",
	Args:  cobra.ExactArgs(2),
	RunE: func(cmd *cobra.Command, args []string) error {
		client, err := newPartnerClient(cmdutil.GetResolvedConfig())
		if err != nil {
			return err
		}

		resp, err := client.RemoveUserFromOrganization(context.Background(), args[0], args[1])
		if err != nil {
			return err
		}

		if isJSONMode() {
			return output.PrintJSON(cmd.OutOrStdout(), resp)
		}

		fmt.Fprintf(cmd.OutOrStdout(), "User %s removed from organization %s\n", args[1], args[0])
		return nil
	},
}

func init() {
	orgUserCmd.AddCommand(orgUserRemoveCmd)
}
