...
#
# Without `identityVerification`, `create_signing_url` still works: `identityVerificationMode` comes
...
begin
  ...
  unless settings["enabled"]
    abort "Embedded signing is not enabled for this organization. An admin can turn it on in E-Signature settings."
  end
  puts "Embedded signing enabled. external_idv allowed: #{settings['allowExternalIdv']}, " \
       "override allowed: #{settings['allowIdentityOverride']}.\n\n"
  ...
  puts "Document #{sent['documentId']} prepared.\n\n"
  ...
  puts "Open this URL for the signer (new tab, redirect, or iframe):"
  puts "  #{link['url']}"
  # With no identityVerification, `identityVerificationMode` is null and `pendingChecks` is [].
  puts "  mode: #{link['identityVerificationMode'] || '(none)'}"
  ...
  puts "  pendingChecks: #{link['pendingChecks'].inspect}"
  puts "  expiresAt: #{link['expiresAt'] || '(no separate expiry; follows the document window)'}"
